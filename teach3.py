...
hidden_layer_weights = [[[0.06, 0.17, 0.12],
                         [0.08, 0.33, 0.18],
                         [0.15, 0.92, 0.12],
                         [0.98, 0.11, 0.20]
                         ],
...
for i in range(num_iterations):
	for i in range(len(data)):
		...
		# Compute gradients
		dz = 2 * (loss - target)  # Common factor: ∂loss/∂z
		dz1 = dz * 出力z_1 * (1 - 出力z_1)
		dz2 = dz * 出力z_2 * (1 - 出力z_2)
		...
		for i in range(len(hidden_layer_weights)):
			for j in range(len(hidden_layer_weights[i])):
				for k in range(len(hidden_layer_weights[i][j])):
					hidden_layer_weights[i][j][k] -= 0.01 * dz1  # Example update rule
		for i in range(len(hidden_layer_weights)):
			for j in range(len(hidden_layer_weights[i])):
				for k in range(len(hidden_layer_weights[i][j])):
					hidden_layer_weights[i][j][k] -= 0.01 * dz2  # Example update rule

		print(f"Iteration {i}: loss = {loss:.6f}, z = {誤差Q:.6f}")
